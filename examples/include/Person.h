...
-itr functions for getting itr to access list's states (list, warning_list, lstm_list)
	...
	-getPersonItrById(int)		*return the i'th element in the list. Return list.end() if i > length of the list. Faster 
...
public:
	...
	void update_cnn_predictions(const char prediction)
	{
		this->cnn_predictions.push_back(prediction);
#ifdef LOG
		std::cout << "Person " << this->id << ": Prediction stored.\nNumber of predictions stored: " << this->cnn_predictions.size() << std::endl;
#endif
		if ((int)cnn_predictions.size() > sz_cnn_predictions)
		{
#ifdef LOG
			std::cout << "Number of predictions stored exceeds " << sz_predictions << ": Remove the eldest." << std::endl;
#endif
			this->cnn_predictions.erase(this->cnn_predictions.begin());
		}
	}
	std::vector<char> get_cnn_predictions() const { return this->cnn_predictions; }
	int num_cnn_predictions() const { return this->cnn_predictions.size(); }
...
private:
	...
	std::vector<char> cnn_predictions;
	int cnt;
	bool found;
};
...
auto get_person_itr(std::vector<Person>& list, const int& id); // return person itr if found; null else
// wtf
...
public:
	...
	auto getPersonItrById(const int& id) const// return person iterator if found, else null iterator is returned
	{
		if (list.empty())
			return list.end();
		auto itr = list.begin();
	...
	auto getPersonItrByPos(const int& pos) const
	{
		if (pos >= list.end() - list.begin())
			return list.end();
		return list.begin() + pos;
	}
	auto itrListBegin() const{ return list.begin(); }
	auto itrListEnd() const{ return list.end(); }
	auto itrWarningListBegin() const{ return warning_list.begin(); }
	auto itrWarningListEnd() const{ return warning_list.end(); }
	auto itrLSTMListBegin() const{ return lstm_list.begin(); }
	auto itrLSTMListEnd() const{ return lstm_list.end(); }
	...
	void printList() const
	{
		...
		for (auto itr = list.begin(); itr != list.end(); itr++)
		{
			std::cout << itr->get_id() << spacing(n_space(4, itr->get_id()))
					  ...
					  << itr->num_cnn_predictions() << "/" << sz_cnn_predictions  << spacing(n_space(3, itr->num_cnn_predictions()) + n_space(3, sz_cnn_predictions))
	...
	torch::Tensor sppe_tns(std::vector<cv::Mat> &imgs) const
	{
		std::vector<torch::Tensor> temp;
		for (auto itr = imgs.begin(); itr != imgs.end(); itr++)
		{
			temp.push_back(torch::from_blob((*itr).clone().data, { 1, 320, 256, 3 }));
		}
		return torch::cat(temp).permute({ 0,3,1,2 }).clone();
	}
	...
	void updateCNNPredictions(int pos, char prediction)
	{
		if (pos >= list.end() - list.begin())
			throw "Exception:Postion Out of Range!";
		list[pos].update_cnn_predictions(prediction);
	}
	...
	cv::Mat &yolo_img(cv::Mat & img, bool greyScale = false) const
	{
		cv::Size sz(416,416);
	...
	torch::Tensor yolo_tns(cv::Mat & img) const
	{
		return torch::from_blob(img.data, { 1, 416, 416, 3 }).permute({0, 3, 1, 2}).clone();
	}
private:
